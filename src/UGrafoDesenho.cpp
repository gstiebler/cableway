...
void CGrafoDesenho::CaboMaisProximo(TPonto &ponto, int &IndiceCabo,
                                    double &DistMaisProx, TPonto &PosVertice, int Diferente, int Nivel)
{
  ...
  if (IndiceCabo==-1)
  {
    ...
    erros->novoErro((string)"Atenção: " + ExtractFileName(Dados->NomeArq.c_str()).c_str() + " não possui cabos.");
    /*    if ( Dados->IMP )
    {
...
// Acha a ponta 1
void CGrafoDesenho::AchaPonta(int &iPonto, int &iM, int &Ponta, TListaItensCelula *ListaItens, double DistMinBandeirola)
{
  int m;
  ...
  do
  {
    ...
    if (IndiceCabo >= 0)
    {
      ...
      if ( fabs(Ponto.x - PontoInicial.x) < DistMinBandeirola && fabs(Ponto.y - PontoInicial.y) < DistMinBandeirola )
      {
  ...
  for (n=0; n<ListaBandeirolas->Tamanho(); n++)
  {
    ...
    for ( m=0; m<ListaItens->Tamanho(); m++ )
    {
      ...
      if ( ListaItens->getItem(m)->TipoVetorCW == VMULTIPOINT )
      {
        ...
        PontosExtremidadesElementosBandeirola.push_back(Dados->Multipoint[IndiceNoVetorMultipoint].pontos[IndiceDoUltimoPonto]);
    ...
    for ( int i = 0 ; i < (int)PontosExtremidadesElementosBandeirola.size() ; i++ )
    {
      ...
      {
        double distancia = DistPontos(PontosExtremidadesElementosBandeirola.at(i), PontosExtremidadesElementosBandeirola.at(j));
    ...
    {
      CErrosMsg *erros = CErrosMsg::getInstance();
      erros->novoErro((string)"Atenção: Há algo em nível de bandeirola fora do formato esperado.");
    ...
    {
      VerticeGeral.texto=Dados->Textos[ListaItens->iTextos->at(0)].texto;
      TPonto MaisDist;
      double DistMaisProx;
      ...
      if ((RetasBandeirola.size()==2) && (NumArcosBandeirola==0))//verifica se a bandeirola possui uma reta horizontal e outra inclinada
      {
        ...
        for (int k=0; k<2; k++)
        {
          DifX=RetasBandeirola.at(k).pontos[0].x-RetasBandeirola.at(k).pontos[1].x;
          DifY=RetasBandeirola.at(k).pontos[0].y-RetasBandeirola.at(k).pontos[1].y;
        ...
        if (MenorInclinacao<0.0001)//pelo menos uma das retas é horizontal
        {
          ...
          for (int k=0; k<2; k++)
          {
            DifY=fabs(RetasBandeirola.at(iMaisInclinado).pontos[k].y-RetasBandeirola.at(iMenosInclinado).pontos[0].y);
            if (DifY>MaiorDifY)
            {
              MaiorDifY=DifY;
              iPontoMaisLonge=k;
      ...
      if (!Bandeirola2Retas)
      {
        TPonto origemTexto =Dados->Textos[ListaItens->iTextos->at(0)].origem;
        origemTexto.x += ( Dados->Textos[ListaItens->iTextos->at(0)].FatorAltura *
                FATOR_FONTE * Dados->Textos[ListaItens->iTextos->at(0)].texto.size() ) *
                cos(Dados->Textos[ListaItens->iTextos->at(0)].rotacao) / 2 ;
        origemTexto.x += Dados->Textos[ListaItens->iTextos->at(0)].FatorAltura *
                FATOR_FONTE * cos(Dados->Textos[ListaItens->iTextos->at(0)].rotacao * M_PI/2)/ 2;
        origemTexto.y += ( Dados->Textos[ListaItens->iTextos->at(0)].FatorAltura *
                FATOR_FONTE * Dados->Textos[ListaItens->iTextos->at(0)].texto.size() ) *
                sin(Dados->Textos[ListaItens->iTextos->at(0)].rotacao) / 2 ;
        origemTexto.y += Dados->Textos[ListaItens->iTextos->at(0)].FatorAltura *
                FATOR_FONTE * sin(Dados->Textos[ListaItens->iTextos->at(0)].rotacao * M_PI/2)/ 2;
    ...
    {
      CErrosMsg *erros = CErrosMsg::getInstance();
      erros->novoErro((string)"No desenho " + ExtractFileName(Dados->NomeArq.c_str()).c_str() +
                  " existe um grupamento em nível de bandeirola com mais de um texto associado. Textos: ");
      for ( int i = 0 ; i < ListaItens->iTextos->size() ; i++ )
        erros->novoErro(Dados->Textos[ListaItens->iTextos->at(i)].texto);
      erros->novoErro("");
    }
...
//percorre lista de cabos ligados ao equipamento
    ...
    {
      bool isAdded;
      isAdded = ListaItensCelula->VerificaSeCaboRetaJaFoiLigadoAoEquipamento(ListaMenores.at(n).IndiceCabo);

      if ( !isAdded )
      {
        ligado = ligaEquipamentoSeDesligado(ListaItensCelula, ligado);
        //adiciona o vértice na lista de vértices do cabo
        CabosReta[ListaMenores.at(n).IndiceCabo]->AdicionaVertice(VerticesGerais->Tamanho(), ListaMenores.at(n).PosVertice);
        ...
        if ( CabosReta[ListaMenores.at(n).IndiceCabo]->EhOPrimeiroPonto(ListaMenores.at(n).PosVertice, Dados->Multipoint, ListaMenores.at(n).IndiceCabo) )
        {
          CabosReta[ListaMenores.at(n).IndiceCabo]->ponta[0] = true;
        }
        else if ( CabosReta[ListaMenores.at(n).IndiceCabo]->EhOUltimoPonto(ListaMenores.at(n).PosVertice, Dados->Multipoint, ListaMenores.at(n).IndiceCabo) )
        {
        ...
        {
          Aresta.AdicionaVertices(iVerticesInstrumento->at(i), VerticesGerais->Tamanho()-1,
...
void CGrafoDesenho::GeraVerticesInstrumentosAdicionaMultipointCaboArco(TListaItensCelula *ListaItensCelula,
                            TVectorPontoEIndiceCabo &ListaMenores, bool &ligado, TVectorInt *iVerticesInstrumento, TPonto PosVertice)
{
    for ( int n = 0 ; n < (int) ListaMenores.size() ; n++ )
    {
      bool isAdded;
      isAdded = ListaItensCelula->VerificaSeCaboArcoJaFoiLigadoAoEquipamento(ListaMenores.at(n).IndiceCabo);
      ...
      if ( !isAdded )
      {
        ...
        CabosArco[ListaMenores.at(n).IndiceCabo]->ponta[ListaMenores.at(n).IndiceArco] = true;
        for ( int i = 0 ; i < ListaItensCelula->iTextos->size() ; i++ )
        {
          Aresta.AdicionaVertices(iVerticesInstrumento->at(i), ListaMenores.at(n).IndiceVertice,
            DistPontosManhattan(PosVertice, PosVertice));
          Aresta.IndiceDesenho=Dados->IndiceDesenho;
          Aresta.IDArquivo=Dados->IDArquivo;
          Arestas->Adiciona(Aresta);
...
void CGrafoDesenho::GeraVerticesInstrumentosAdicionaMultipoint(int Indice, TListaItensCelula *ListaItensCelula, bool &ligado, TVectorInt *iVerticesInstrumento, TPonto PosVertice)
{
  ...
  for (j=0; j<MultipointT.tam; j++)
  {
    ...
    GeraVerticesInstrumentosAdicionaMultipointCaboReta(ListaItensCelula, ListaMenores, ligado, iVerticesInstrumento, PosVertice);
    ...
    GeraVerticesInstrumentosAdicionaMultipointCaboArco(ListaItensCelula, ListaMenores, ligado, iVerticesInstrumento, PosVertice);

  }
}
//---------------------------------------------------------------------------

void CGrafoDesenho::GeraVerticesInstrumentosAdicionaArco(int Indice, TListaItensCelula *ListaItensCelula, bool &ligado, TVectorInt *iVerticesInstrumento, TPonto PosVertice)
{
  TVectorPontoEIndiceCabo ListaMenores;
  TArco Arco;
  int IndiceCabo;
...
//    DistMaisPerto = ListaMenores[n].Dist;
    ...
    if ( !isAdded )
    {
      ...
      if ( CabosReta[IndiceCabo]->EhOPrimeiroPonto(PosVertice, Dados->Multipoint, IndiceCabo) )
      {
        CabosReta[IndiceCabo]->ponta[0] = true;
      }
      else if ( CabosReta[IndiceCabo]->EhOUltimoPonto(PosVertice, Dados->Multipoint, IndiceCabo) )
      {
...
void CGrafoDesenho::GeraVerticesInstrumentos()
{
  ...
  for (n=0; n<Dados->InfoCelula.ListaCelulasInstrumentos->Tamanho(); n++)//percorre todas as células do desenho
  {
    ...
    {
      CErrosMsg *erros = CErrosMsg::getInstance();
      erros->novoErro((string)"No desenho " + ExtractFileName(Dados->NomeArq.c_str()).c_str() + " existe um grupamento em nível de equipamento sem textos associados.\n");
      continue;
    }

    PosVertice = AchaPosVerticeInstrumento(ListaItensCelula);
    CriaVerticesEArestasInstrumento (ListaItensCelula, iVerticesInstrumento, PosVertice, PrensaCabo);

    for (i=0; i<ListaItensCelula->Tamanho(); i++)//percorre todos os itens da célula atual
    {
      ItemCelula=ListaItensCelula->getItem(i);
      switch (ItemCelula->TipoVetorCW)//verifica o tipo do elemento do equipamento
      {
      case VMULTIPOINT:  //adiciona um vértice de um instrumento na ponta de um cabo
        GeraVerticesInstrumentosAdicionaMultipoint(ItemCelula->Indice, ListaItensCelula, ligado, iVerticesInstrumento, PosVertice);
        break;
      case VARCO:
        GeraVerticesInstrumentosAdicionaArco(ItemCelula->Indice, ListaItensCelula, ligado, iVerticesInstrumento, PosVertice);
        break;
...
void CGrafoDesenho::CriaVerticesEArestasInstrumento (TListaItensCelula *ListaItensCelula, TVectorInt *iVerticesInstrumento, TPonto PosVertice, bool PrensaCabo)
{
  ...
  for ( int i = 0 ; i < ListaItensCelula->iTextos->size() ; i++ )
  {
    ...
    VerticeInstrumento.texto=Dados->Textos[ListaItensCelula->iTextos->at(i)].texto;
  ...
  for (i=0; i<ListaItensCelula->Tamanho(); i++)
  {
    ...
    if ( ListaItensCelula->getItem(i)->TipoVetorCW == VMULTIPOINT )
    {
      for (j=0; j<Dados->Multipoint[ListaItensCelula->getItem(i)->Indice].tam; j++)
      {
        ...
        {
          first = false;
          maiorX = menorX = Dados->Multipoint[ListaItensCelula->getItem(i)->Indice].pontos[j].x;
          maiorY = menorY = Dados->Multipoint[ListaItensCelula->getItem(i)->Indice].pontos[j].y;
        }
...
void CGrafoDesenho::GeraVerticesArcos()
{
  ...
  DistMaisProx = 0;
  PontoNaReta.x = PontoNaReta.y = PontoTemp.x = PontoTemp.y = p[0].x = p[0].y = p[1].x = p[1].y = 0.0;
  for (n=0; n<NumCabosArco; n++)
  {
    ...
    {                                    
      ...
      //no outro cabo arco
      DistPontoParaPontaCaboArco(p[m], IndiceCabo, DistMaisProx, PontoTemp, IndiceVertice, n, PontaArco);
      ...
      else//se o vértice não foi aproveitado de outro cabo arco, verifica se este vértice está próximo a um cabo reta
      {
        ...
        if (DistMaisProx<DIST_MIN_ELEM_CABO)
        {
          ...
          VerticeGeral.pos=PontoNaReta;
          CabosArco[n]->ponta[m] = true;
          if ( CabosReta[iMenorDist]->EhOPrimeiroPonto(PontoNaReta, Dados->Multipoint, iMenorDist) )
          {
            CabosReta[iMenorDist]->ponta[0] = true;
          }
          else if ( CabosReta[iMenorDist]->EhOUltimoPonto(PontoNaReta, Dados->Multipoint, iMenorDist) )
          {
    ...
    if (Arco->Estilo==0)//SE O ARCO FOR TRACEJADO, OU SEJA, ESTÁ NO CHÃO, SOMA A ALTURA DO TETO
      ...
      Aresta.AdicionaVertices(Arco->iV[0], Arco->iV[1], DistPontosManhattan(p[0], p[1])+AlturaTeto);
    Aresta.IndiceDesenho=Dados->IndiceDesenho;
    Aresta.IDArquivo=Dados->IDArquivo;
    Arestas->Adiciona(Aresta);
  }
...
void CGrafoDesenho::DistRetaParaPontaCaboReta(TPonto Reta[2],
...
void CGrafoDesenho::DistRetaParaPontaCaboArco(TPonto Reta[2],
...
void CGrafoDesenho::DistArcoParaTodasPontasRetaCabo(TArco &Arco, TVectorPontoEIndiceCabo &ListaMenores, double DistMinElemCabo)
{
  int m, n;
  unsigned char ContadorMaior;
  double Dist;
  ...
  for (m=0; m<NumCabosReta; m++)
  {
    ...
    for (n=0; n<2; n++)
    {
      ...
      Dist=fabs(sqrt(pow(Arco.Centro.x-PontosCabo[n].x, 2)+
        pow(Arco.Centro.y-PontosCabo[n].y, 2))-Arco.EixoPrimario);
      if (Dist<DistMinElemCabo)
      {
...
//ESTA FUNÇÃO DEVE SE ADAPTAR PARA TRABALHAR COM ARCOS QUE NÃO SÃO CÍRCULOS
void CGrafoDesenho::DistArcoParaPontaArcoCabo(TArco &Arco, int &IndiceArcoCabo, double &DistMaisPerto,
                                              TPonto &PosVertice)
{
  int m, n;
  unsigned char ContadorMaior;
...
void CGrafoDesenho::GeraColares(std::vector<void*> ListaDesenhos)
{
  ...
  for (n=0; n< (int) (Lista->size()-1); n++)
  {
    ...
    {
      V2=Lista->at(n+1);
      if ( (V1->texto==V2->texto && !V2->EhPrensaCabo) && (V1->iDesenho!=V2->iDesenho) && (V1->TipoElemento==INSTRUMENTO) && (V2->TipoElemento==INSTRUMENTO))
      {
        double alturaDaAresta = ( (TDesenho *) (ListaDesenhos[V1->iDesenho]))->Altura - ( (TDesenho *) (ListaDesenhos[V2->iDesenho]))->Altura;
        if ( alturaDaAresta < 0 )
          alturaDaAresta *= -1;
        TAresta Aresta;
        ...
        VerticesGerais->getItem(V1->IndiceOriginal)->EhColar = VerticesGerais->getItem(V2->IndiceOriginal)->EhColar = true;
      }
    }
  }
  ...
  for (n=0; n< (int)(Lista->size()-1); n++)
  {
    ...
    if (V1->texto!="")
    {
      ...
      if (V1->texto==V2->texto)
      {
        ...
        if ( V1->iDesenho != V2->iDesenho && ( V1->TipoElemento == INSTRUMENTO && V2->TipoElemento == INSTRUMENTO ) && (n+2 < (int)(Lista->size()-1) && (Lista->at(n+1))->texto != V1->texto ) )
        {
          n++;
          break;
        }
        ...
        for ( ; n < (int)Lista->size(); n++)
        {
          if ( (Lista->at(n))->texto == V1->texto )
          {
            ...
            erros->novoErro("No desenho: " + ((TDesenho *)(ListaDesenhos[((TVerticeGeral *)(Lista->at(n)))->iDesenho]))->NomeArquivo + " em nível de " + tipo);
          }
          else
            break;
        }
